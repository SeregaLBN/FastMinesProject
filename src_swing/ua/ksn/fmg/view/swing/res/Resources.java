...
public final class Resources {
	...
	private Map<EMosaic         , ImageIcon> imgsMosaic, imgsMosaicWide;
	...
	public Image getImgLogo() {
		if (imgLogo == null)
			imgLogo = getImage("Logo/MinesWeeper.png");
		return imgLogo;
	}
	...
	public ImageIcon getImgPause() {
		if (imgPause == null) {
			imgPause = getImageIcon("Field/BackgroundPause.png"); // сначала из ресурсов
			if (imgPause == null)
...
//	}
	...
	public ImageIcon getImgMosaic(EMosaic key) {
		if (imgsMosaic == null) {
			imgsMosaic = new HashMap<EMosaic, ImageIcon>(EMosaic.values().length);

			for (EMosaic val: EMosaic.values()) {
				ImageIcon imgMosaic = getImageIcon("Mosaic/32x32/" + val.getDescription(true)+".png"); // сначала из ресурсов
	...
	public ImageIcon getImgMosaic(EMosaic key, int newWidth, int newHeight) {
		ImageIcon original = getImgMosaic(key);
		if (original == null) return null;
		return ImgUtils.toImgIco(original.getImage(), newWidth, newHeight);
	}

 	public ImageIcon getImgMosaicWide(EMosaic key) {
		if (imgsMosaicWide == null) {
			imgsMosaicWide = new HashMap<EMosaic, ImageIcon>(EMosaic.values().length);

			for (EMosaic val: EMosaic.values()) {
				ImageIcon imgMosaic = getImageIcon("Mosaic/48x32/" + val.getDescription(true)+".png"); // сначала из ресурсов
				if (imgMosaic == null) // иначе - своя картинка из кода
					imgMosaic = ImgUtils.toImgIco(ImgUtils.toImg(new MosaicsImg(val, false)));
				imgsMosaicWide.put(val, imgMosaic);
			}
		}
		return imgsMosaicWide.get(key);
	}
	public ImageIcon getImgMosaicWide(EMosaic key, int newWidth, int newHeight) {
		ImageIcon original = getImgMosaicWide(key);
		if (original == null) return null;
		return ImgUtils.toImgIco(original.getImage(), newWidth, newHeight);
	}
