...
//		((JSpinner.DefaultEditor)spinMines.getEditor()).getTextField().getDocument().addDocumentListener(new SpinNumberDocListener(spinMines));
		...
		if (parent != null)
			parent.getMosaic().addMosaicListener(new MosaicListener() {
				...
				@Override
				public void OnChangeMosaicType(MosaicEvent e) { if (CustomSkillDlg.this.isVisible()) CustomSkillDlg.this.OnChangeMosaicType(); }
				@Override
				public void OnChangeGameStatus(MosaicEvent e) {}
				@Override
				public void OnChangeCounters(MosaicEvent e) {}
				@Override
				public void OnChangeArea(MosaicEvent e) { if (radioFullScreenCurrSizeArea.isSelected()) radioGroup.clearSelection(); }
			});
	}
