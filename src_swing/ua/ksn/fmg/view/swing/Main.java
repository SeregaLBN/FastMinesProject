...
import ua.ksn.fmg.view.swing.draw.EShowElement;
import ua.ksn.fmg.view.swing.draw.EZoomInterface;
import ua.ksn.fmg.view.swing.draw.GraphicContext;
import ua.ksn.fmg.view.swing.event.ActionToUser;
			...
			private JRadioButtonMenuItem getMenuItemMosaic(EMosaic key) {
				if (mosaics == null) {
					...
					for (EMosaic val: EMosaic.values()) {
						...
						menuItem.setIcon(Main.this.getResources().getImgMosaic(val, icoMosaicWidht, icoMosaicHeight));
		...
		public MosaicListener getMosaicListener() {
			if (mosaicListener == null)
				mosaicListener = new MosaicListener() {
					@Override
					public void OnClick(MosaicEvent e) {
//						System.out.println("OnMosaicClick: down=" + e.isDown() + "; leftClick=" + e.isLeftClick());
					...
					@Override
					public void OnChangeGameStatus(MosaicEvent e) {
...
//						System.out.println("OnChangeGameStatus: " + e.getSource().getGameStatus());
					...
					@Override
					public void OnChangeCounters(MosaicEvent e) {
					...
					@Override
					public void OnChangeArea(MosaicEvent e) {
						Main.this.ChangeSizeImagesMineFlag();
					}

					@Override
					public void OnChangeMosaicType(MosaicEvent e) {
...
//	}
	...
	public void setStatisticAndChampion(MosaicEvent e) {
		if (e.getSource().getGameStatus() != EGameStatus.eGSEnd)
			throw new RuntimeException("Invalid method state call");

		// сохраняю все нужные данные
		final boolean victory = e.getSource().isVictory();
		...
		final EMosaic eMosaic = e.getSource().getMosaicType();
		final long realCountOpen = e.getRealCountOpen();
		final long playTime = Long.parseLong(Main.this.getToolbar().getEdtTimePlay().getText());
		final long clickCount = e.getSource().getCountClick();
