...
//		g.drawRect(rcInner.x, rcInner.y, rcInner.width, rcInner.height);
	...
	public void paintBorderLines(BaseCell cell, Graphics g) {
		if (gContext.isIconicMode()) {
			g.setColor(cell.getState().isDown() ? gContext.getPenBorder().getColorLight() : gContext.getPenBorder().getColorShadow());
			...
			for (int i=0; i<v; i++) {
				...
				if (i==s)
					g.setColor(cell.getState().isDown() ? gContext.getPenBorder().getColorShadow(): gContext.getPenBorder().getColorLight ());
...
//		g.drawRect(rcInner.x, rcInner.y, rcInner.width, rcInner.height);
		...
		// output text
		{
			...
			if (cell.getState().getStatus() == EState._Close) {
				g.setColor(gContext.getColorText().getColorClose(cell.getState().getClose().ordinal()));
...
//				szCaption = ""+cell.getDirection(); // debug
				...
				g.setColor(gContext.getColorText().getColorOpen(cell.getState().getOpen().ordinal()));
