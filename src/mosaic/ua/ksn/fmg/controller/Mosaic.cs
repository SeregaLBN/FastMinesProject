...
public abstract class Mosaic {
   ...
   /// <summary>площадь одной ячейки</summary>
         ...
         if (recreateMatrix) {
            ...
            for (int i=0; i < size.width; i++)
               for (int j=0; j < size.height; j++) {
                  ...
                  attr.OnChangeArea += cell.OnPropertyAreaChange; // подписываю новые ячейки на уведомления атрибута (изменение a -> перерасчёт координат)
               }
