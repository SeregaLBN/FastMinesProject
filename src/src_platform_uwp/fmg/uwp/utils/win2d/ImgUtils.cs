...
namespace fmg.uwp.utils.win2d {
   ...
   public class ImgUtils {
      ...
      public static async Task<CanvasBitmap> GetImage(Uri uri) {
      ...
      /// <summary> загрузить картинку из локальных ресурсов </summary>
      private static async Task<CanvasBitmap> LoadBitmap(Uri uri) {
         try {
      ...
      /// <summary> загрузить картинку из локальных ресурсов </summary>
      private static async Task<CanvasBitmap> LoadBitmap(string resourceName) {
      ...
      public static CanvasBitmap Zoom(CanvasBitmap img, int newWidth, int newHeight) {
         if (img == null)
            return null;
         ...
         var device = CanvasDevice.GetSharedDevice();
         var tmp = new CanvasRenderTarget(device, newWidth, newHeight, dpi);
