...
void CAssistant::Work() {
   ...
   //if (lastClick != INCORRECT_COORD) IMAGE_JOB_SET;
   nsMosaic::CClickData click;
   if (IsSequentialMoveProcessed()) { // выполняется ли перебор флажков?
      SequentialMove(click); // да, выполняется - продолжить перебор флажков
   } else {
      if (!FindCell(click)) { // найти ячейку для клика
         //IMAGE_JOB_RESET;
         return; // работа была прервана
      }
      if (click.m_CoordCell == INCORRECT_COORD) {
         if (!SequentialMoveCanBegin()) { // проверка условий для начала перебора флажков
            //IMAGE_JOB_RESET;
            return; // работа была прервана
         }
         if (IsSequentialMoveProcessed()) {       // выполнять ли перебор флажков?
            SequentialMove(click);       // да, выполнять - начать перебор флажков
         } else {
            if (m_Mosaic.GetGameStatusIsPlay() &&
