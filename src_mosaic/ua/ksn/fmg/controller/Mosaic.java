...
//		public void setMosaicType(EMosaic newMosaicType) { setParams(null, newMosaicType, null); }
		...
		public void setParams(Size newSizeField, EMosaic newMosaicType, Integer newMinesCount) {
			...
			getMosaicListeners().fireOnChangeCounters(new MosaicEvent(Mosaic.this));
			if (newMosaciType)
				getMosaicListeners().fireOnChangeMosaicType(new MosaicEvent(Mosaic.this));
		}
...
//			{ // old algorithm
//				firstClickCell.LockNeighborMines(); // запрещаю установку мин у соседей и у себя
//				int count = 0;
//				Random rand = new Random();
//				do {
//					Size sizeField = getCells().getSize();
//					int i = Math.abs(rand.nextInt(sizeField.x));
//					int j = Math.abs(rand.nextInt(sizeField.y));
//					if (getCell(i,j).getState().SetMine())
//						count++;
...
//			}
			...
			{ // new algorithm
				...
				do {
					...
					if (cellToSetMines.getState().SetMine()) {
						...
						OnError("В новом алгоритме мины должны всегда устанавливаться...");
				} while (count < minesCount);
			}
	...
	public void setGameStatus(EGameStatus newStatus, Integer realCountOpen) {
		_gameStatus = newStatus;
		getMosaicListeners().fireOnChangeGameStatus((realCountOpen==null) ? new MosaicEvent(this) : new MosaicEvent(this, realCountOpen));
	}
	...
	/**  подписаться на уведомления изменений свойств мозаики */
	public void addPropertyChangeListener(PropertyChangeListener l) {
		getPropertyChangeSupport().addPropertyChangeListener(l);
	}
	/**  отписаться от уведомлений изменений свойств мозаики */
...
//			listeners2.remove(MosaicListener.class, l);
		...
		public void fireOnClick(MosaicEvent e) {
			for (MosaicListener l: getListeners())
				l.OnClick(e);
		}
		/** уведомить об о изменении счётчика таймера/мин */
		public void fireOnChangeCounters(MosaicEvent e) {
			for (MosaicListener l: getListeners())
				l.OnChangeCounters(e);
		}
		/** уведомить об изменении статуса игры (новая игра, начало игры, конец игры) */
		public void fireOnChangeGameStatus(MosaicEvent e) {
			for (MosaicListener l: getListeners())
				l.OnChangeGameStatus(e);
		}
		/** уведомить о изменении размера площади у ячейки */
		public void fireOnChangeArea(MosaicEvent e) {
			for (MosaicListener l: getListeners())
				l.OnChangeArea(e);
		}
		/** уведомить о изменении размера площади у ячейки */
		public void fireOnChangeMosaicType(MosaicEvent e) {
			for (MosaicListener l: getListeners())
	...
	protected void GameBegin(Coord firstClick) {
		...
		setGameStatus(EGameStatus.eGSPlay, null);
...
//			::SetCursor(::LoadCursor(NULL, IDC_ARROW));
		...
		setGameStatus(EGameStatus.eGSEnd, realCountOpen);
		getMosaicListeners().fireOnChangeCounters(new MosaicEvent(this));
	}
	...
	protected void OnLeftButtonDown(Coord coordLDown) {
		...
		if (coordLDown.equals(Coord.INCORRECT_COORD))
			return;
		getMosaicListeners().fireOnClick(new MosaicEvent(this, true, true));
	...
	protected void OnLeftButtonUp(Coord coordLUp) {
		...
		if (getCoordDown().equals(Coord.INCORRECT_COORD))
			return;
		getMosaicListeners().fireOnClick(new MosaicEvent(this, true, false));
		if (getGameStatus() == EGameStatus.eGSCreateGame)
			return;
...
//		System.out.println("OnLeftButtonUp: coordLUp="+coordLUp);
	...
	protected void OnRightButtonDown(Coord coordRDown) {
		...
		if (coordRDown.equals(Coord.INCORRECT_COORD))
			return;
		getMosaicListeners().fireOnClick(new MosaicEvent(this, false, true));
...
//				m_GContext.m_hDCDst, 0, 0, SRCCOPY);
	...
	protected void OnRightButtonUp(/*Coord coordRUp*/) {
		getMosaicListeners().fireOnClick(new MosaicEvent(this, false, false));
	}
...
//		System.out.println("Mosaic::GameNew()");
		...
		setGameStatus(EGameStatus.eGSReady, null);
	...
	public void GameCreate() {
		...
		if (getRepositoryMines().isEmpty()) {
			...
			setGameStatus(EGameStatus.eGSCreateGame, null);
			getMosaicListeners().fireOnChangeCounters(new MosaicEvent(this));
		}
	}
...
//	private void setCell(Coord coord, BaseCell cell) { mosaic.get(coord.x).set(coord.y, cell); }
	...
	public void setArea(int newArea)  {
		...
		getMosaicListeners().fireOnChangeArea(new MosaicEvent(this));
	}
	...
	private void resetCountClick()     { _countClick=0; getMosaicListeners().fireOnChangeCounters(new MosaicEvent(this)); }
	private void incrementCountClick() { _countClick++; getMosaicListeners().fireOnChangeCounters(new MosaicEvent(this)); }
