...
public class MosaicViewController extends MosaicController<DrawableView, MosaicViewView> {
    ...
    /** мин отступ от краев экрана для мозаики */
...
//        control.setOnCapturedPointerListener(null);
    ...
    private SizeDouble getOffset() {
        return model.getMosaicOffset();
    }
    private void setOffset(SizeDouble offset) {
        model.setMosaicOffset(recheckOffset(offset));
    }
    ...
    private double calcMaxArea() {
        ...
        SizeDouble sizeIn = model.getSize();
        SizeDouble sizeOut = new SizeDouble();
        double area = MosaicHelper.findAreaBySize(model.getMosaicType(), mosaicSizeField, sizeIn, sizeOut);
    ...
    private void beforeZoom(PointDouble mouseDevicePosition) {
        if (mouseDevicePosition != null) {
            ...
            zoomStartInfo.mosaicSize = model.getSizeMosaic();
        } else {
            zoomStartInfo = null;
        }
    }
    ...
    private void afterZoom() {
        ...
        SizeDouble mosaicSizeNew = model.getSizeMosaic();//GetMosaicSize(Model.SizeField, Model.Area);
    ...
    void zoomInc(double zoomPower/* = 1.3 */, PointDouble mouseDevicePosition/* = null */) {
        ...
        if (deferredZoom) {
            ...
            double newArea = model.getArea() * 1.01 * zoomPower;
            model.setArea(Math.min(newArea, calcMaxArea()));
            afterZoom();
        }
    }
    ...
    void zoomDec(double zoomPower/* = 1.3 */, PointDouble mouseDevicePosition/* = null */) {
        ...
        if (deferredZoom) {
            ...
            double newArea = model.getArea() * 0.99 / zoomPower;
            model.setArea(Math.max(newArea, MosaicInitData.AREA_MINIMUM));
            afterZoom();
        }
    }
    ...
    private void onControlSizeChanged(Size newSize) {
        if (!bindSizeDirection)
            model.setSize(Cast.toSizeDouble(newSize));
    }
    ...
    private void onModelSizeChanged() {
        ...
        SizeDouble newSize = model.getSize();
        if (newSize == null)
            newSize = model.getSize();
        lp.width  = (int)newSize.width;
        lp.height = (int)newSize.height;
    }

    private void onModelAreaChanged() {
        if (!extendedManipulation)
            return;
        try (Logger.Tracer tracer = new Logger.Tracer("Mosaic.onModelAreaChanged", String.format(Locale.US, "newArea=%.2f", model.getArea()))) {
    ...
    private boolean onGestureDoubleTap(MotionEvent ev) {
        ...
        try (Logger.Tracer tracer = new Logger.Tracer("Mosaic.onGestureDoubleTap", "ev=" + motionEventToString(ev), () -> "handled="+handled[0]))
        {
            ...
            if (rcMosaic.contains(ev.getX(), ev.getY())) {
                ...
                // 2. modify offset
                mosaicSize = model.getSizeMosaic(); // ! reload value
