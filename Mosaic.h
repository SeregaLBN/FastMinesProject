...
namespace nsMosaic {
   ...
   struct CSerialize {
      COORD   m_SizeMosaic; // размер пол€ (в €чейках)
      int     m_iArea;      // площадь
   ...
   public:
      ...
      void        SetGame(EMosaic mosaic, const COORD& newSizeMosaic, int numberMines, const CStorageMines *pStorageCoordMines); // установить мозаику заданного размера и с определЄнным количеством мин (координаты мин могут задаватьс€ с помощью "’ранилища ћин")
      ...
      SIZE  (*GetSizeWindow) (const COORD& sizeField, int area);            // размер в пиксел€х
      COORD   GetSize()       const {return m_SerializeData.m_SizeMosaic;} // размер в €чейках
