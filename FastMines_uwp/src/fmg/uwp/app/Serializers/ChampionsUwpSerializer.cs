...
namespace Fmg.Uwp.App.Serializers {
    ...
    /// <summary> Champions (de)serializer </summary>
    public class ChampionsUwpSerializer : ChampionsSerializer {
        ...
        private async Task<byte[]> Read(StorageFile file) {
            using (Stream stream = (await file.OpenReadAsync()).AsStreamForRead()) {
                using (BinaryReader input = new BinaryReader(stream, Encoding.UTF8)) {
                    ...
                    if (version != VERSION)
                        throw new Exception("Invalid file data. Unsupported " + nameof(Champions) + " version " + version);
        ...
        protected override string GetChampionsFile() {
            return AProjSettings.ChampionsFileName;
        }
